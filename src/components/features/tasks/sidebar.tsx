            ...
            {filterOptions.map((option) => {
              ...
              return (
                <Button
                  ...
                  className={`${sidebarOpen ? "w-full justify-start" : "w-12 h-12 p-0 justify-center"}`}
              ...
              <div className="space-y-1">
                {priorityOptions.map((priority) => (
                  <Button
                    ...
                    onClick={() =>
                      dispatch(setFilters({ priority: priority.value }))
                    }
                  >
                    <div
                      className={`w-3 h-3 rounded-full ${getPriorityColor(priority.value).bg}`}
                    />
            ...
            <div className="space-y-1">
              {categories.map((category) => (
                <Button
                  key={category.id}
                  variant={
                    filters.categoryId === category.id ? "secondary" : "ghost"
                  }
                  className={`${sidebarOpen ? "w-full justify-start" : "w-12 h-12 p-0 justify-center"}`}
                  onClick={() =>
